...
import os
import pickle
from mySOT.tracker.features.config import OTBHCConfig
...
class Feature:
    ...
    def _sample_patch(self, im, pos, sample_sz, output_sz):
        ...
        x_max = min(im.shape[1], int(xs.max()))
        y_min = max(0, int(ys.min()))
        y_max = min(im.shape[0], int(ys.max()))
        # extract image
...
class TableFeature(Feature):
    def __init__(self, fname, compressed_dim, table_name, use_for_color, cell_size=1):
        super(TableFeature, self).__init__(OTBHCConfig)
        self.fname = fname
    ...
    def integral_vec_image(self, img):
        w, h, c = img.shape
